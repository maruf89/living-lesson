...
function html5blank_header_scripts()
{
    if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
        ...
        wp_register_script('main', get_template_directory_uri() . '/main.js', array('jquery'), '1.0.0'); // Custom scripts
...
function loop_showcase($category = 'classtivity') {
    global $categories;
    $cat = 'cat=' . $categories[$category];
    $custom_query = new WP_Query($cat);

    while($custom_query->have_posts()) {
        $custom_query->the_post();
        get_template_part('elements/class-item');
    } 
