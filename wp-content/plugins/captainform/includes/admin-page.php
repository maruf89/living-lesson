...
function captainform_page_handler()
{
	...
	if ($installation_id == '')
	{
		...
		add_site_option($GLOBALS['captainform_option1'], $installation_id);
	}
	...
	if ($installation_key == '')
	{
		...
		add_site_option($GLOBALS['captainform_option2'], $installation_key);
	}
	...
	if (isset($_SESSION['devdebug']) && ($_SESSION['devdebug'] == 'ON'))
		$wp_seturl = $wp_protocol . "://" . $GLOBALS['captainform_servicedomain'];
	else
		$wp_seturl = $wp_protocol . "://" . $wp_subid . "." . $GLOBALS['captainform_servicedomain'];

	$wp_seturl = strtolower($wp_seturl);
	$url = $wp_seturl . "/fh-connect.php?inst=" . captainform_wpp_encrypt($installation_id) . "&key=" . captainform_wpp_encrypt($installation_key) . "&site=" . captainform_wpp_encrypt($site) . "&display_name=" . captainform_wpp_encrypt($display_name) . "&page=$page" . "&is_multisite=" . var_export(captainform_get_site_type(), true) . captainform_get_ref_param() . "&wp_version=" . captainform_wpp_encrypt($captainform_plugin_version);
...
{
	global $captainform_wpp_options;

	ob_start();
	?>
	<div class="wrap">
		<h2>CaptainForm WordPress Plugin Options</h2>
	</div>
	<?php
	echo ob_get_clean();
}
...
function captainform_insert_dialog()
{
	...
	if ($response->status == 'ok')
	{
		...
		<div class="captainform_widget_container">
			...
			<br>
			<select name="<?php echo $GLOBALS['captainform_plugin_name']; ?>_form_toembed" id="<?php echo $GLOBALS['captainform_plugin_name']; ?>_form_toembed" class="captainform_widget_select">
				<?php
				$first_form_id = null;
		...
		?>
				<br />
				<input type="hidden" id="captainform_publish_code" name="<?php if(isset($captainform_lightbox_publish_code_name)) echo $captainform_lightbox_publish_code_name; ?>" class="cf_generated_code" value="<?php echo $captainform_publish_code_value; ?>" />
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
		</div>
